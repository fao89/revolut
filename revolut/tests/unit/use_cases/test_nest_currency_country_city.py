...
def test_add_one_currency():
    currency = ParseJson(
        [{"country": "UK", "city": "London", "currency": "GBP", "amount": 12.2},], NEST
    )
...
def test_add_two_countries_with_same_currency():
    ...
    currency.parse()
    assert currency.output == {
        "EUR": {"ES": {"Madrid": [{"amount": 8.9}]}, "FR": {"Paris": [{"amount": 20}]}},
    }
...
def test_add_two_cities_with_same_country_for_three_nests():
    ...
    currency.parse()
    assert currency.output == {
        "EUR": {"FR": {"Lyon": [{"amount": 11.4}], "Paris": [{"amount": 20}]},},
    }
...
def test_add_two_cities_with_same_country_for_two_nests():
    ...
    assert currency.output == {
        "EUR": {
            "FR": [{"amount": 11.4, "city": "Lyon"}, {"amount": 20, "city": "Paris"},]
        },
    }
